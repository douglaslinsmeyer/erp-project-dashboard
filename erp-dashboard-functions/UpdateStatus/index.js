...
module.exports = async function (context, req) {
    try {
        ...
        // Get current status
        const statusBlobClient = containerClient.getBlockBlobClient('current-status.json');
        ...
        const uploadBlobResponse = await statusBlobClient.upload(content, content.length, {
            overwrite: true
        });
...
async function saveHistory(containerClient, historyEntry) {
    ...
    const historyBlobClient = containerClient.getBlockBlobClient(historyPath);
    ...
    await historyBlobClient.upload(historyContent, historyContent.length, {
        overwrite: true
    });
}
